...
BIN_PATH = join(sublime.packages_path(), dirname(realpath(__file__)), 'jsprettier.js')
...
class FormatJavascriptCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        if not self.has_selection() or settings.get('ignore-selection', False):
            ...
            formated = self.jsprettier(originalBuffer, self.get_scope(region))
            if formated:
    ...
    def jsprettier(self, data, scope):
        try:
            ...
            return node_bridge(data, BIN_PATH, cdir, [opt, scope])
